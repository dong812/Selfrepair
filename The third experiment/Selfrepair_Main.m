...
%             if Flag.SelfRepair
                ...
                for h = beg_Syn : end_Syn
                    ...
                    deltaNeuCa = Par.Ts .*(-NeuVar.CA2Store(h,j-1)/NeuPar.tau_Ca)+Input_Spikes(h,j-1).*NeuPar.C_pre+NeuVar.Spikes(n,j-1).*NeuPar.C_post+AstVar.localCA2_Spike(h,j-1).*NeuPar.C_SIC;
                    NeuVar.CA2Store(h,j)= NeuVar.CA2Store(h,j-1)+deltaNeuCa;
                    rpSpikes = 0;
                    rdSpikes = 0;
                    if NeuVar.CA2Store(h,j-1)>= NeuPar.theta_p
                       rpSpikes = 1;
                       rdSpikes = 1;
                      
                    elseif NeuVar.CA2Store(h,j-1)>= NeuPar.theta_d
                            rdSpikes = 1;
                        
                    else
                        NeuVar.omega(h,j) =NeuVar.omega(h,j-1);
                    end
                    rou= NeuVar.rou(h,j-1);
                    deltarou = Par.Ts .*((-rou.*(1-rou).*(0.5-rou)+rpSpikes.*NeuPar.r_p.*(1-rou)-rdSpikes.*NeuPar.r_d.*rou))/NeuPar.tau_rou;
                    NeuVar.rou(h,j)=NeuVar.rou(h,j-1)+deltarou;
                    %deltaomega =
                    %NeuVar.omega(h,j) =NeuVar.omega(h,j-1)+ deltarou.*(NeuVar.omega_1-NeuVar.omega_0);
                    NeuVar.omega(h,j) =NeuVar.rou(h,j-1).*(NeuVar.omega_1-NeuVar.omega_0)+300;
                end
...
while counter < z
    ...
    for i = 1:5
        subplot(5,1,i)
        plot(Par.t, NeuVar.omega(i+counter,:))
        if i == 1
