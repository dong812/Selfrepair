...
%             if Flag.SelfRepair
                ...
                for h = beg_Syn : end_Syn
                 ...
                 %Change in Glutamate (equation 14, Wade et al 2012)
                      ...
                      deltaEsp = Par.Ts .* ((-AstVar.Esp(h)/AstPar.tau_Esp) + ((AstVar.Esp_weight.*(AstVar.Glu(k)+AstVar.localGluStore(h,j))/AstPar.tau_Esp)));
...
Cathresholds = 0.2.*ones(1,400000);

figure
subplot(2,1,1)
plot(Par.t,AstVar.CA2Store(11,:),Par.t,Cathresholds)
ylabel('Ca^2^+(\muM)')
xlabel('Time (s)')
subplot(2,1,2)
plot(Par.t,AstVar.CA2Store(20,:),Par.t,Cathresholds)
ylabel('Ca^2^+(\muM)')
xlabel('Time (s)')


%Plots and saves the results
%Plot1(Flag, Par, AstVar, SynVar, freq)